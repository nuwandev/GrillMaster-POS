/**
 * @fileoverview New Order screen - Select order type and customer.
 */
...
/** Maximum number of customers to display in the list. */
const MAX_CUSTOMER_DISPLAY = 12;

/**
 * New Order screen controller.
 */
export class NewOrderScreen {
  ...
  /**
   * Render order type selection section.
    ...
    return `
      <div data-order-type-section class="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow p-5 md:p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          ${
            this.selectedType
              ? '<span class="text-xs text-green-600 font-medium">✓ Selected</span>'
  ...
  renderOrderTypeButton(type, emoji, label) {
    ...
    return `
      <button 
        ...
        class="relative py-6 md:py-8 px-3 md:px-6 rounded-xl border-2 transition-all group ${
          isSelected
            ? 'border-blue-500 bg-blue-50 text-blue-700 shadow-md scale-105'
            : 'border-gray-200 bg-white text-gray-700 hover:border-blue-300 hover:bg-blue-50/50 hover:scale-105'
        }"
      >
        ${
          isSelected
            ? '<div class="absolute top-2 right-2 w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center"><span class="text-white text-xs">✓</span></div>'
            : ''
        }
        <div class="text-3xl md:text-4xl mb-2 transition-transform ${
  ...
  /**
   * Render customer selection section.
   * @returns {string} Section HTML
   */
    ...
    return `
      <div data-customer-section class="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow p-5 md:p-6">
        <div class="flex items-center justify-between mb-3">
          ...
          ${
            selectedCustomer
              ? `<span class="text-xs text-green-600 font-medium">✓ ${selectedCustomer.name}</span>`
        ...
        <div class="relative">
          <input 
            ...
            value="${this.query}"
            oninput="newOrderScreen.handleSearchInput(this)"
          ...
          ${
            this.query
              ? '<button onclick="newOrderScreen.clearSearch()" class="absolute right-3 top-3.5 text-gray-400 hover:text-gray-600">✕</button>'
              : ''
          }
        </div>
  ...
  renderCustomerActions() {
    ...
    return `
      <div data-customer-actions class="flex flex-wrap gap-2 mt-3">
        <button 
          onclick="newOrderScreen.selectGuest()" 
        ...
        <button 
          onclick="newOrderScreen.toggleAddForm()" 
        ...
        ${
          this.query && !this.showAddForm && filteredCustomers.length === 0
            ? `
          <button 
            onclick="newOrderScreen.toggleAddForm('${this.query.replace(/"/g, '&quot;')}')" 
            class="px-4 py-2 rounded-lg bg-blue-500 text-white border-2 border-blue-500 hover:bg-blue-600 transition-all text-sm font-medium shadow-sm"
          >
            ✨ Add "${this.query}"
          </button>
        `
      ...
      <div class="mt-4 p-4 border-2 border-blue-200 rounded-xl bg-blue-50/50">
        ...
        <div class="space-y-3">
          <input
            ...
            placeholder="Name *"
            value="${this.newName}"
            oninput="newOrderScreen.setNewName(this.value)"
          ...
          <input
            ...
            placeholder="Phone"
            value="${this.newPhone}"
            oninput="newOrderScreen.setNewPhone(this.value)"
            class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none"
          />
          <input
            type="email"
            placeholder="Email"
            value="${this.newEmail}"
            oninput="newOrderScreen.setNewEmail(this.value)"
            class="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:outline-none"
          />
          <div class="flex gap-2">
            <button
              onclick="newOrderScreen.saveNewCustomer()"
              class="flex-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 font-medium"
            >
              Save
            </button>
            <button
              onclick="newOrderScreen.cancelAddForm()"
  ...
  renderCustomerResults() {
    ...
    if (filteredCustomers.length === 0) {
      if (this.query && !this.showAddForm) {
        return '<div class="mt-4 text-center py-8 text-gray-400 text-sm">No customers found</div>';
      }
      return '';
    }
    ...
    return `
      <div class="mt-4">
        <div class="text-xs font-medium text-gray-500 mb-2 px-2">
          ${this.query ? 'SEARCH RESULTS' : 'RECENT CUSTOMERS'}
        </div>
  ...
  renderCustomerItem(customer) {
    ...
    return `
      <div 
        onclick="newOrderScreen.selectCustomer('${customer.id}')"
  ...
  renderStartButton() {
    return `
      <div class="sticky bottom-0 pt-4 pb-2 bg-gradient-to-t from-gray-100 via-gray-100 to-transparent">
        <button 
          data-start-btn
          onclick="newOrderScreen.startOrdering()"
          class="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white text-xl md:text-2xl font-bold py-5 md:py-6 rounded-2xl shadow-lg hover:shadow-xl hover:from-blue-700 hover:to-blue-800 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
  ...
  getFilteredCustomers() {
    ...
    if (!query) {
      return store.state.customers
        .filter((cust) => cust.name !== 'Guest')
        .slice(0, MAX_CUSTOMER_DISPLAY);
    }

    return store.state.customers.filter(
      (cust) =>
        cust.name.toLowerCase().includes(query) || cust.phone.includes(query)
    );
  }
   ...
   * @param {string} [prefillName] - Prefill name value
   */
  ...
  /**
   * Start ordering - navigate to POS.
   */
  startOrdering() {
